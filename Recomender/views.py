...
def getmaxrating(user_id):
  ...
  usernew=user_df.groupby("Title").agg({"rating": "max", "timestamp": "max"}).sort_values("rating", ascending=False).head(5)
  return usernew.index

def getmaxfilm():
  wibk = getfilm()
  filmMax = wibk.groupby("Title").agg({"vote_average": "max"}).sort_values("vote_average", ascending=False).head(10)
...
class expertMovie(APIView):
   def get(self,request):
      ...
      # writer = csv.writer(response)
          ...
          for std in studs:
              writer.writerow(str(str(s), 'utf-8') for s in std)
